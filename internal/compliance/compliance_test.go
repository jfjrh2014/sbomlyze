...
func TestEvaluate_NTIAMissingFields(t *testing.T) {
	comps := []sbom.Component{
		{Name: "a"}, // minimal: has name, nothing else
	}
	info := sbom.SBOMInfo{} // no tool, no schema
	...
	// Should fail on: version, supplier, unique-id, author, timestamp
	// Should pass on: name, dep-relation (single component = no deps needed = pass)
	...
	comps := []sbom.Component{
		{
			Name: "a", Version: "1.0", Supplier: "dev",
			PURL: "pkg:npm/a@1.0", Licenses: []string{"MIT"},
			Hashes: map[string]string{"SHA-256": "abc123"}, // only SHA-256, no SHA-512
			Dependencies: []string{"b"},
		},
	}
	info := sbom.SBOMInfo{ToolName: "syft", SchemaVersion: "1.0"}
...
func TestEvaluate_MultipleComponents(t *testing.T) {
	comps := []sbom.Component{
		...
		{Name: "b", Version: "2.0"}, // missing supplier, PURL, license
	}
	info := sbom.SBOMInfo{ToolName: "syft", SchemaVersion: "1.0"}
	...
	// Should have less than 100 because some components are incomplete
	if report.NTIA.Score == 100 {
...
func TestEvaluate_CISAComponentChecksCount(t *testing.T) {
	...
	// CISA = NTIA base (7) + PURL + License + Hash + CPE = 11
...
func TestEvaluate_BSIComponentChecksCount(t *testing.T) {
	...
	// BSI = 11 base checks (name, version, creator, unique-id, purl, license, hash, sha512, deps) + 2 SBOM = 11 total
