...
func evaluateNTIA(comps []sbom.Component, info sbom.SBOMInfo) *FrameworkResult {
	...
	withDeps := countWith(comps, func(c sbom.Component) bool {
		// NTIA requires the *relationship* to be described. Components that declare
		// their dependencies satisfy this. Components without dependencies also
		// satisfy it if they're leaf nodes or root nodes in the graph.
		return len(c.Dependencies) > 0 || total <= 1
	})

	// SBOM-level checks (boolean)
	hasAuthor := info.ToolName != ""
	// NTIA says timestamp should be present, but we check schema version as proxy
	// since the model doesn't have a dedicated timestamp field
